...
from .ucscexception import UcscValidationException, UcscWarning
from .ucsccore import UcscBase
...
class ManagedObject(UcscBase):
    ...
    def __init__(
        ...
        UcscBase.__init__(self, ucscgenutils.word_u(xml_attribute))

        if self.__parent_mo:
            self.__parent_mo.child_add(self)

        if kwargs:
            for prop_name, prop_value in ucscgenutils.iteritems(kwargs):
    ...
    def __setattr__(self, name, value):
        ...
        if "prop_meta" in dir(self) and name in self.prop_meta:
            ...
            # These are properties which the current version of ucscsdk
            # does not know of.
            # The code will come here lot often, when using older version of
            # ucscsdk with newer releases on the UcsCentral.
    ...
    def set_prop_multiple(self, **kwargs):
        for prop_name in kwargs:
            if prop_name not in self.prop_meta.keys():
                UcscWarning("Invalid Property Name for "
    ...
    def __str__(self):
        ...
        for prop, prop_value in sorted(ucscgenutils.iteritems(
                self.__dict__)):
    ...
    def show_hierarchy(self, level=0, depth=None, show_level=[]):
        ...
        from .ucsccoreutils import print_mo_hierarchy
...
class GenericMo(UcscBase):
    ...
    def __init__(self, class_id, parent_mo_or_dn=None, **kwargs):
        ...
        UcscBase.__init__(self, class_id)

        if kwargs:
            for key, value in ucscgenutils.iteritems(kwargs):
    ...
    def to_mo(self):
        ...
        class_id = ucscgenutils.word_u(self._class_id)
        if class_id not in ucscmeta.MO_CLASS_ID:
            return None
        ...
        for prop in self.__properties:
            if prop in mo.prop_map:
                ...
                UcscWarning("Property %s Not Exist in MO %s" % (
                    ucscgenutils.word_u(prop), class_id))
    ...
    def __str__(self):
        ...
        if isinstance(self, GenericMo):
            ...
            for prop, prop_val in sorted(ucscgenutils.iteritems(
                    self.__dict__)):
