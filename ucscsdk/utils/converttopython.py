...
from ..ucscconstants import Status, NamingPropertyId, YesOrNo
...
def _get_class_id_for_rn(rn, prev_class_id=None):
    ...
    meta_class_id = ucsccoreutils.find_class_id_in_mo_meta_ignore_case(
        prev_class_id)
    ...
    mo_meta = ucsccoreutils.get_mo_property_meta(meta_class_id, "mo_meta")
    if mo_meta is None:
        raise UcscValidationException(
    ...
    for child_class_id in mo_meta.children:
        ...
        child_class_id = ucsccoreutils.find_class_id_in_mo_meta_ignore_case(
            child_class_id)
        child_mo_meta = ucsccoreutils.get_mo_property_meta(child_class_id,
                                                          "mo_meta")
...
def _create_property_map_from_node(class_node, class_status):
    ...
    if not gmo_flag:
        ...
        peer_class_ref = ucsccoreutils.load_class(peer_class_id)
    ...
    naming_props_py_from_rn = ucsccoreutils.get_naming_props(
...
def _get_config_conf_cmdlet(node, is_pair_node):
    ...
    for mo in import_list:
        mo_folder = re.match("([a-z])+", ucscgenutils.word_l(mo)).group()
        import_cmdlet += "from ucscsdk.mometa.%s.%s import %s\n" % (
            mo_folder, mo, mo)
...
def _form_configconf_cmdlet(class_node, key, tag, import_list, parent_tag=None,
    ...
    if class_status & _ClassStatus.GET == _ClassStatus.GET:
        ...
        if class_status & _ClassStatus.MODIFIED == _ClassStatus.MODIFIED:
            op_flag = "addmodify"
            add_prop_map = ", ".join(
                [k + "=" + v for k, v in ucscgenutils.iteritems(property_map)])
        ...
        op_flag = "set"
        set_prop_map = "\n".join(
            [tag + "." + k + " = " + v for k, v in ucscgenutils.iteritems(property_map)])
        cmdlet = "%s = handle.query_dn(\"%s\")\n%s\n" % (
...
def _generate_config_resolve_cmdlet(xml_string, method):
    ...
    # <configResolveClasses>
        ...
        cmdlet = '\nfrom ucscsdk.ucsbasetype import ClassIdSet, ClassId\n'
...
def _generate_single_clone_cmdlets(xml_string, is_template):
    ...
    if is_template:
        ...
        cmdlet = '\nfrom ucscsdk.ucsmethodfactory import ls_clone\n'
...
def _generate_ls_templatise_cmdlets(xml_string):
    ...
    if dest_org is not None:
        cmdlet = '\nfrom ucscsdk.ucsmethodfactory import ls_templatise\n'
...
def _generate_multiple_clone_cmdlets(xml_string, is_prefix_based):
    ...
    if is_prefix_based:
        ...
        cmdlet = '\nfrom ucscsdk.ucsmethodfactory import ' \
        ...
        cmdlet += 'from ucscsdk.ucsbasetype import DnSet, Dn\n\n'
        cmdlet += "dn_set = DnSet()" + "\n"
...
def _generate_config_conf_rename_cmdlet(xml_string):
    ...
    cmdlet = "\nfrom ucscsdk.ucsmethodfactory import config_conf_rename\n\n"
...
def convert_to_ucs_python(xml=False, request=None,
                          ...
                          dump_xml=False):
    """
    This operation generates python script from xml request.
    ...
    if _outfile_flag in ucscgenutils.AFFIRMATIVE_LIST:
        if _outfile_path:
