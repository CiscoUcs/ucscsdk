...
from ucscsdk.ucscgenutils import download_file, get_md5_sum, \
        Progress
...
def get_ucsc_cco_image_list(username=None, password=None,
    ...
    Returns:
        List of UcscCcoImage objects

    Example:
        image_list = get_ucsc_cco_image_list("username", "password")
    """
    ...
    for image_node in image_node_list:
        ...
        for property_node in property_node_list:
            ...
            if property_node.getAttribute(
                    "name") == _UcscCcoImageList.IDAC_TAG_VERSION:
                image.version = property_node.getAttribute("value")
                continue
            if property_node.getAttribute(
                    "name") == _UcscCcoImageList.IDAC_TAG_IMAGE_NAME:
                image.image_name = property_node.getAttribute("value")
                continue
            if property_node.getAttribute(
                    "name") == _UcscCcoImageList.IDAC_TAG_URL:
                image.url = property_node.getAttribute("value")
                continue
            if property_node.getAttribute(
                    "name") == _UcscCcoImageList.IDAC_TAG_IP_URL:
                image.ip_url = property_node.getAttribute("value")
                continue
            if property_node.getAttribute(
                    "name") == _UcscCcoImageList.IDAC_TAG_SIZE:
                image.size = int(property_node.getAttribute("value"))
                continue
            if property_node.getAttribute(
                    "name") == _UcscCcoImageList.IDAC_TAG_CHECKSUM:
            ...
            if property_node.getAttribute("name") == \
                    _UcscCcoImageList.IDAC_TAG_FILE_DESCRIPTION:
...
def get_ucsc_cco_image(image, file_dir, proxy=None, progress=Progress()):
    """
    Downloads image from CCO

    Args:
        image (UcscCcoImage): object of type UcscCcoImage
    ...
    Example:
        image_list = get_ucsc_cco_image_list("username", "password")\n
        get_ucsc_cco_image(image=image_list[0],
                           file_dir="/home/user/images")\n
    """

    if not image:
        raise UcscValidationException("Provide an image to be downloaded")

    if not isinstance(image, UcscCcoImage):
        raise UcscValidationException("Object is not of type UcscCcoImage")

    if not file_dir:
        raise UcscValidationException("Provide file_dir to be downloaded")
    ...
    try:
        driver = UcscDriver(proxy)
        driver.add_header("Authorization",
                          "Basic %s" % image.network_credential)
        ...
        raise UcscOperationError(
            "Download of Image", "download failed")

    local_file = os.path.join(file_dir, str(image.image_name))

    if not os.path.exists(local_file):
        raise UcscValidationException("URL parameter is not provided.")

    md5_sum = get_md5_sum(local_file)
    if not md5_sum:
        UcscWarning("Unable to generate md5sum for file <%s>" % local_file)
        UcscWarning("Deleting file <%s> ....." % local_file)
        os.remove(local_file)
        return
    log.debug(md5_sum)
    log.debug(image.checksum_md5)
    if md5_sum != image.checksum_md5:
        UcscWarning("Incorrect md5sum for file <%s>" % local_file)
        UcscWarning("Deleting file <%s> ....." % local_file)
        os.remove(local_file)
        return
