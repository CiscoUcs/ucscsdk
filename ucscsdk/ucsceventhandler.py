...
from .ucscexception import UcscWarning
...
class UcscEventHandle(object):
    ...
    def _process_event_channel_resp(self, resp):
        ...
        for mo_elem in self._get_mo_elem(resp):
            gmo = ucscmo.generic_mo_from_xml_elem(mo_elem[0])
    ...
    def _read_msg_length(self, msg):
        '''
        The first line of the resp msg contains the length of the xml resp
        being received. This method extracts and returns the lenth of the msg
        from the xml response.
        Note: readline() from socket buffer is not working reliably across
        all python versions
        '''
        msg_len = ""
        while True:
    ...
    def _enqueue_function(self):
        ...
        try:
            while self._can_enqueue():
                msg_len = self._read_msg_length(self._event_chan_resp)
                resp = self._event_chan_resp.read(msg_len)
    ...
    def _get_ucsc_prop_name(self, mo, python_prop):
        if isinstance(mo, ucscmo.GenericMo):
            return python_prop
    ...
    def _prop_val_match(self, mo, prop, success_values,
                        change_list=None):
        ucsc_prop = self._get_ucsc_prop_name(mo, prop)
        prop_val = self._get_prop_value(mo, prop)

        if self._should_skip_prop_match(ucsc_prop, change_list):
            return False
    ...
    def _dequeue_mo_prop_poll(self, mo, prop, poll_sec, watch_block,
        ...
        if pmo is None:
            UcscWarning('Mo ' + pmo.dn + ' not found.')
            return
    ...
    def _add_class_id_watch(self, class_id):
        if ucsccoreutils.find_class_id_in_mo_meta_ignore_case(class_id) is \
                None:
    ...
    def _add_mo_watch(self, managed_object, prop=None, success_value=[],
                      poll_sec=None):
        if ucsccoreutils.find_class_id_in_mo_meta_ignore_case(
        ...
        if prop is not None:
            mo_property_meta = ucsccoreutils.get_mo_property_meta(
    ...
    def add(self,
        ...
        subscribe for the event channel from UcsCentral and can monitor those
        events for any specific success value for a managed object.
        ...
        if filter_callback is None:
            raise UcscValidationException("Error adding WatchBlock...")
    ...
    def remove(self, watch_block):
        ...
        if watch_block in self._wbs:
            ...
            UcscWarning("Event handler not found")
        self._wbs_lock.release()
...
def wait(handle, mo, prop, value, cb, timeout_sec=None, poll_sec=None):
    ...
    Args:
        handle(UcscHandle): connection handle to the server
    ...
    Example:
        This method is called from UcscHandle class,
