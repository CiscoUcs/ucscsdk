...
"""
This module contains the ucscsdk Core utilities.
"""
...
from . ucscmeta import MO_CLASS_ID, METHOD_CLASS_ID, \
...
def get_ucsc_obj(class_id, elem, mo_obj=None):
    """
    ...
    mo_obj = ucscmo.GenericMo(class_id=elem.tag,
...
def load_module(module_name):
    ...
    module_name = ucscgenutils.word_u(module_name)
...
def load_class(class_id):
    ...
    class_id = ucscgenutils.word_u(class_id)
    if class_id and class_id in MO_CLASS_ID:
        mod_class_id = ucscgenutils.word_l(class_id)
...
def write_object(mo_or_list):
    ...
    if isinstance(mo_or_list, ucscmethod.ExternalMethod):
        if hasattr(mo_or_list, "out_configs"):
            for child in mo_or_list.out_configs.child:
                if isinstance(child, ucscmo.ManagedObject):
                    write_object(child)
    elif isinstance(mo_or_list, list) and len(mo_or_list) > 0:
        for mo in mo_or_list:
            if (isinstance(mo, ucscmo.ManagedObject) or
                    isinstance(mo, ucscmo.GenericMo)):
                print(mo)
    elif (isinstance(mo_or_list, ucscmo.ManagedObject) or
            isinstance(mo_or_list, ucscmo.GenericMo)):
        print(mo_or_list)
...
class ClassIdMeta(object):
    ...
    def __init__(
            ...
            show_tree=True,
            depth=None):
        from ucscsdk import ucsccoremeta
        ...
        if include_prop:
            ...
            for prop in sorted(self.props):
                ...
                if self.props[prop].access == \
                        ucsccoremeta.MoPropertyMeta.READ_WRITE:
...
def _show_tree(class_id, depth=None, level=0, ancestor_str="",
    ...
    meta_class_id = ucscgenutils.word_u(class_id)

    out_str = ""
    if not ancestor:
        for parent in sorted(MO_CLASS_META[meta_class_id].parents):
            out_str += "[" + ucscgenutils.word_u(parent) + "]" + "\n"
...
def search_class_id(class_id):
    ...
    l_class_id = class_id.lower()
    class_ids = sorted([cid for cid in ucscmeta.MO_CLASS_ID
