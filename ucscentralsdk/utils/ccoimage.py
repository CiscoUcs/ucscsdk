...
class UcscentralCcoImage(object):
    """
...
def get_ucscentral_cco_image_list(username=None, password=None,
    ...
    Returns:
        List of UcscentralCcoImage objects
    ...
    if not mdf_id_list:
        for mdf_id in ucscentral_mdf_ids:
    ...
    for image_node in image_node_list:
        ...
        for property_node in property_node_list:
            ...
            if property_node.getAttribute(
                    "name") == _UcscentralCcoImageList.IDAC_TAG_VERSION:
                image.version = property_node.getAttribute("value")
                continue
            if property_node.getAttribute(
                    "name") == _UcscentralCcoImageList.IDAC_TAG_IMAGE_NAME:
                image.image_name = property_node.getAttribute("value")
                continue
            if property_node.getAttribute(
                    "name") == _UcscentralCcoImageList.IDAC_TAG_URL:
                image.url = property_node.getAttribute("value")
                continue
            if property_node.getAttribute(
                    "name") == _UcscentralCcoImageList.IDAC_TAG_IP_URL:
                image.ip_url = property_node.getAttribute("value")
                continue
            if property_node.getAttribute(
                    "name") == _UcscentralCcoImageList.IDAC_TAG_SIZE:
                image.size = int(property_node.getAttribute("value"))
                continue
            if property_node.getAttribute(
                    "name") == _UcscentralCcoImageList.IDAC_TAG_CHECKSUM:
...
def get_ucscentral_cco_image(image, file_dir, proxy=None, progress=Progress()):
    ...
    Args:
        image (UcscentralCcoImage): object of type UcscentralCcoImage
    ...
    if md5_sum != image.checksum_md5:
        UcsCentralWarning("Incorrect md5sum for file <%s>" % local_file)
        UcsCentralWarning("Deleting file <%s> ....." % local_file)
        os.remove(local_file)
        return
