...
class UcsCentralEventHandle(object):
    ...
    def _get_ucscentral_prop_name(self, mo, python_prop):
    ...
    def _prop_val_match(self, mo, prop, success_values,
                        change_list=None):
        ucscentral_prop = self._get_ucscentral_prop_name(mo, prop)
        prop_val = self._get_prop_value(mo, prop)

        if self._should_skip_prop_match(ucscentral_prop, change_list):
            return False
    ...
    def _add_class_id_watch(self, class_id):
        if ucscentralcoreutils.find_class_id_in_mo_meta_ignore_case(class_id) is None:
...
def wait(handle, mo, prop, value, cb, timeout_sec=None, poll_sec=None):
    ...
    Args:
        handle(UcsCentralHandle): connection handle to the server
    ...
    Example:
        This method is called from UcsCentralHandle class,
