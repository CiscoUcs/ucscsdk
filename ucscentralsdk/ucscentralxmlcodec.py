...
from . import ucscentralexception as ex
...
def from_xml_str(xml_str, handle=None):
    ...
    if root_elem.tag == "error":
        ...
        raise ex.UcsCentralException(error_code, error_descr)

    class_id = ucscentralgenutils.word_u(root_elem.tag)
    response = ucscentralcoreutils.get_ucs_central_obj(class_id, root_elem)
